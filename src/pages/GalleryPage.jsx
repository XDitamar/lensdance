...
// Helper: path לתמונה מוקטנת
// MainGallery/xxx.jpg  -> MainGallery/thumbs/xxx.jpg
const getThumbRefPath = (fullPath) => {
  if (
    fullPath.startsWith("MainGallery/") &&
    !fullPath.startsWith("MainGallery/thumbs/")
  ) {
...
const LazyMedia = React.memo(function LazyMedia({
  ...
  const [loaded, setLoaded] = useState(!lqipUrl); // אם אין LQIP – נחשב כ-loaded
      ...
      {inView ? (
        isVideo ? (
          <video
            ...
            src={url} // וידאו – אותו URL (אין thumb בנפרד כאן)
          ...
          <>
            {/* LQIP – רק אם עדיין לא loaded ויש lqipUrl */}
            {!loaded && lqipUrl && (
              <img
                src={lqipUrl}
...
export default function GalleryPage() {
  // unified: { name, fullPath, ext, source, url (full), gridUrl (thumb), isVideo, variant, lqipUrl }
          ...
          try {
            ...
            firebaseRefs = await Promise.all(
              res.items.map(async (r) => {
                ...
                // ברירת מחדל – לגריד נשתמש ב-full אם אין thumb
                ...
                if (!isVid) {
                  ...
                  const cachedThumb = urlCache.current.get(thumbPath);
                  const thumbUrl =
                    cachedThumb ||
                    (await getDownloadURL(thumbRef).catch(() => null));

                  if (thumbUrl) {
                    urlCache.current.set(thumbPath, thumbUrl);
                    gridDisplayUrl = thumbUrl; // תמונה קטנה לגריד
                    lqipUrl = thumbUrl; // גם כ-LQIP
                  } else {
                    // אם אין thumb – נשתמש ב-full
                    lqipUrl = fullResUrl;
                  }
                } else {
                  // וידאו – רק full
  ...
  // 6) Warm all images (לא חובה, אבל משפר אחרי זמן)
  useEffect(() => {
      ...
      const urls = (
        await Promise.all(
          refsChunk.map(async (r) => {
            ...
            if (
              urlCache.current.has(path) ||
              urlCache.current.has(r.fullPath)
            )
              return urlToWarm;
      ...
      <div className="gallery-grid collage">
        {loadingPage
          ...
          : patternedPageItems.map((m) => (
              <LazyMedia
                key={m.fullPath}
                url={m.gridUrl} // כאן יוצג ה-thumbnail (איכות נמוכה / 1980 וכד')
                alt={m.name}
                isVideo={m.isVideo}
                variant={m.variant}
                lqipUrl={m.lqipUrl}
        ...
        >
          ...
          >
            {selectedItem.isVideo ? (
              ...
              <img
                src={selectedItem.url} // קובץ מלא 4K / 1980 / מה שיש
                alt="תצוגה מוגדלת"
                className="modal-media"
              />
            )}
