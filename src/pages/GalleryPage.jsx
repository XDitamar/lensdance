...
/* ---------- Lazy tile (loads only when in view) - MODIFIED FOR LQIP ---------- */
const LazyMedia = React.memo(function LazyMedia({ url, alt, isVideo, variant, onClick }) {
  const [inView, setInView] = useState(false);
  const [loaded, setLoaded] = useState(false); // New: Tracks if high-res image is loaded
  const [videoSrc, setVideoSrc] = useState("");
  const mediaRef = useRef(null);

  // NOTE: For a real performance gain, the 'url' should point to a tiny, compressed, 
  // blurred thumbnail (LQIP) initially, and then switch to the full URL. 
  // Since the media structure here doesn't provide a separate LQIP URL, 
  // we'll use the main URL for both, relying on your server/Firebase to be fast 
  // or the browser to cache it, but optimally, you'd generate a low-res version.
  const lqipUrl = isVideo ? undefined : url; // Treat the image URL as the placeholder URL
...
            {/* This image is used as a highly compressed, blurred background while the main image loads */}
            {!loaded && lqipUrl && (
              <img
                src={lqipUrl}
...
                aria-hidden="true"
                // Inline styles to ensure visual effect and correct overlay
                style={{
                  filter: 'blur(10px)',
                  opacity: loaded ? 0 : 1, // Hide placeholder when main image is loaded
...
                opacity: loaded ? 1 : 0.01, // Use 0.01 instead of 0 for some browser optimisations
              }}
            />
          </>
        )
